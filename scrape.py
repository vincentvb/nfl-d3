...
for idx, row in enumerate(rows[1:]):
    ...
    winner_points = int(
        next(x for x in data if x.get('data-stat') == 'pts_win').text)
    loser_points = int(
        next(x for x in data if x.get('data-stat') == 'pts_lose').text)
    ...
    day = next(x for x in data if x.get('data-stat')
               == 'game_day_of_week').text.upper()
    ...
    event_parsed = re.split('(AM|PM)', next(
        x for x in data if x.get('data-stat') == 'gametime').text)
